...
int s21_sum_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  ...
  if (is_incorrect(A) || is_incorrect(B))
    ...
    ret = 2;
  else if (s21_create_matrix(A->rows, A->columns, result) != 0) ret = 1;
  else {
...
int s21_mult_number(matrix_t *A, double number, matrix_t *result) {
  ...
  if (is_incorrect(A))
    ret = 1;
  else if (s21_create_matrix(A->columns, A->rows, result) != 0) ret = 1;
  else
    for (int i = 0; i < A->rows; i++)
      for (int j = 0; j < A->columns; j++) {
...
int s21_mult_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  ...
  if (is_incorrect(A) || is_incorrect(B))
    ...
    ret = 2;
  else if (s21_create_matrix(A->columns, B->rows, result) != 0) ret = 1;
  else
    for (int i = 0; i < A->rows; i++)
...
int s21_calc_complements(matrix_t *A, matrix_t *result) {
  int ret = 0;
  if (is_incorrect(A)) ret = 1;
  else if (A->columns != A->rows) ret = 2;
  else if (s21_create_matrix(A->columns, A->rows, result) != 0) ret = 1;
  else {
    matrix_t temp;
